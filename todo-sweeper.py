...
# The idea is to sweep all finished tasks to done.txt and the
# unfinished tasks in the list to the current day.
...
from g_utils import colorz, AMBER, YELLOW, WHITE, RED, BRIGHT, GREEN, GREY50
...
def sweep(todo_filename, done_filename):
    "Parse todos from filename, sweep done into done.txt"

    todoList = parse_one_todo_file(todo_filename)

    with open(done_filename, 'w') as done_f:
        for task in todoList:
            if task.lineNumber == 0:
                print ""
                print >>done_f, ""
                print task.string(color=True)
                print >>done_f, task.string(color=True)
            stat = task.status
            if stat == 'x' or stat == '.':
                print >>done_f, task.string(withDate=False, color=True)
            else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(\
        description='Sweep done tasks out of todo.txt files into done.txt')

    parser.add_argument('-d', '--directory', default='~/txt/todo')
    parser.add_argument('-v', '--verbose', type=int, default=1)
    parser.add_argument('-D', '--done-file', default='~/txt/todo/done.txt')
    parser.add_argument('-t', '--todo-file', nargs='?',
                        default='~/txt/todo/today.txt',
                        help='todo.txt file to sweep to done.txt')
    ...
    sweep(os.path.expanduser(args.todo_file),
          os.path.expanduser(args.done_file))
